...
class KiteApp:
    ...
    def __init__(self, enctoken):
        ...
        self.access_token = {"Authorization": f"token api_key:{self.enctoken}"}
    ...
    def mf_holdings(self):
        mf_holdings = self.session.get(f"{self.root_url}/mf/holdings", headers=self.headers).json()["data"]
        return mf_holdings
    ...
    def place_order(
        ...
        resp = self.session.post(
            f"{self.root_url}/orders/{variety}",
            data=params,
        ...
        # No response at all
        if resp is None:
            raise Exception("No response from Zerodha (session expired or network issue)")
        ...
        try:
            ...
            raise Exception(
                "Non-JSON response from Zerodha (likely login/session expired):\n"
                + resp.text[:500]
            )

        # Zerodha explicit rejection
        if result.get("status") != "success":
            raise Exception(f"Order rejected by Zerodha:\n{result}")
